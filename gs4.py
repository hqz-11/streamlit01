...
if check_password():
    ...
    keyword= df['カテゴリ'].unique().tolist()
    mondai = df['利用者からの質問'].unique().tolist()
    k_select = st.sidebar.selectbox("カテゴリを選択してください", keyword)
    m_select = st.sidebar.selectbox("問題を選択してください", mondai)
    result_df = df[(df['カテゴリ'] == k_select) & (df['利用者からの質問'] == m_select)]
    ...
    if len(result_df) == 0:
        ...
        for i in range(len(result_df)):
            st.write(result_df["チャットボットの回答（高橋様修正後）"].values[i])
